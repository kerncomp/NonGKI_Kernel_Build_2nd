From b32ca42bd04a97f4f62d9f07639db8bea5fa8304 Mon Sep 17 00:00:00 2001
From: midori01 <lv@lvlv.lv>
Date: Sun, 30 Aug 2026 17:24:28 +0900
Subject: [PATCH] Import de-inlined hooks SUSFS v2.3.0

Signed-off-by: midori01 <lv@lvlv.lv>
---
 ...
 include/linux/susfs.h     |  263 ++++++
 include/linux/susfs_def.h |  163 ++++
 kernel/kallsyms.c         |   31 +
 kernel/sys.c              |    8 +
 mm/memory.c               |   11 +
 20 files changed, 3350 insertions(+), 4 deletions(-)
...
index 0e16a9980c6a..34f34cbeb70e 100644
...
index 060d20d737a0..fd53c439bce4 100644
--- a/fs/Makefile
+++ b/fs/Makefile
...
index b3621138c5c6..06969f9cd92f 100644
--- a/fs/namei.c
+++ b/fs/namei.c
@@ -39,6 +39,9 @@
...
+#endif
 	...
 	int		last_type;
 	unsigned	depth;
@@ -524,6 +538,9 @@ static void set_nameidata(struct nameidata *p, int dfd, struct filename *name)
...
@@ -1622,6 +1639,14 @@ static struct dentry *lookup_dcache(const struct qstr *name,
...
@@ -1638,6 +1663,9 @@ static struct dentry *__lookup_hash(const struct qstr *name,
...
+#endif
 	...
 	if (dentry)
 		return dentry;
@@ -1647,6 +1675,9 @@ static struct dentry *__lookup_hash(const struct qstr *name,
...
@@ -1655,6 +1686,26 @@ static struct dentry *__lookup_hash(const struct qstr *name,
 		dput(dentry);
 		dentry = old;
 	}
+
...
@@ -1666,6 +1717,9 @@ static int lookup_fast(struct nameidata *nd,
...
@@ -1676,6 +1730,16 @@ static int lookup_fast(struct nameidata *nd,
 		unsigned seq;
 		bool negative;
...
+#endif
 		if (unlikely(!dentry)) {
 			if (unlazy_walk(nd))
 				return -ECHILD;
@@ -1722,6 +1786,16 @@ static int lookup_fast(struct nameidata *nd,
...
+#endif
 		if (unlikely(!dentry))
 			return 0;
 		status = d_revalidate(dentry, nd->flags);
@@ -1753,12 +1827,19 @@ static struct dentry *__lookup_slow(const struct qstr *name,
...
@@ -1768,6 +1849,12 @@ static struct dentry *__lookup_slow(const struct qstr *name,
 				if (!error) {
 					d_invalidate(dentry);
 					dput(dentry);
+#ifdef CONFIG_KSU_SUSFS_SUS_PATH
...
+#endif
 				...
 				dput(dentry);
@@ -1782,6 +1869,20 @@ static struct dentry *__lookup_slow(const struct qstr *name,
 			dentry = old;
 		}
 	}
+#ifdef CONFIG_KSU_SUSFS_SUS_PATH
...
@@ -1912,6 +2013,11 @@ static int walk_component(struct nameidata *nd, int flags)
 	if (unlikely(err <= 0)) {
 		if (err < 0)
 			return err;
+#ifdef CONFIG_KSU_SUSFS_SUS_PATH
...
@@ -2185,6 +2291,15 @@ static int link_path_walk(const char *name, struct nameidata *nd)
 	for(;;) {
 		u64 hash_len;
 		int type;
+#ifdef CONFIG_KSU_SUSFS_SUS_PATH
...
+#endif
 		...
 		err = may_lookup(nd);
 		if (err)
@@ -2373,6 +2488,9 @@ static inline int lookup_last(struct nameidata *nd)
 {
...
@@ -3313,14 +3431,38 @@ static int lookup_open(struct nameidata *nd, struct path *path,
...
@@ -3444,6 +3586,9 @@ static int do_last(struct nameidata *nd,
 	struct path path;
 	int error;
...
@@ -3653,9 +3798,30 @@ static int do_tmpfile(struct nameidata *nd, unsigned flags,
...
@@ -3678,24 +3844,57 @@ static int do_tmpfile(struct nameidata *nd, unsigned flags,
...
@@ -3714,8 +3913,31 @@ static struct file *path_openat(struct nameidata *nd,
...
+#endif // #ifdef CONFIG_KSU_SUSFS_OPEN_REDIRECT
 	if (likely(!error)) {
 		if (likely(file->f_mode & FMODE_OPENED))
 			return file;
@@ -4939,6 +5161,10 @@ int readlink_copy(char __user *buffer, int buflen, const char *link)
  *
...
@@ -4948,7 +5174,18 @@ int vfs_readlink(struct dentry *dentry, char __user *buffer, int buflen)
...
+#endif
 		...
 		if (!d_is_symlink(dentry))
 			return -EINVAL;
@@ -4964,6 +5201,15 @@ int vfs_readlink(struct dentry *dentry, char __user *buffer, int buflen)
 		if (IS_ERR(link))
 			return PTR_ERR(link);
 	}
+#ifdef CONFIG_KSU_SUSFS_OPEN_REDIRECT
...
index 7dd0fb27bd6e..ac285c99209f 100644
--- a/fs/namespace.c
+++ b/fs/namespace.c
@@ -37,11 +37,19 @@
...
@@ -178,6 +186,11 @@ static int mnt_alloc_id(struct mount *mnt)
...
@@ -186,8 +199,19 @@ static void mnt_free_id(struct mount *mnt)
  */
...
@@ -236,6 +260,111 @@ int mnt_get_count(struct mount *mnt)
 #endif
 }
...
+
 ...
 {
 	struct mount *mnt = kmem_cache_zalloc(mnt_cache, GFP_KERNEL);
@@ -723,6 +852,23 @@ struct mount *__lookup_mnt(struct vfsmount *mnt, struct dentry *dentry)
...
@@ -1069,7 +1215,18 @@ struct vfsmount *vfs_create_mount(struct fs_context *fc)
...
@@ -1193,9 +1350,34 @@ static struct mount *clone_mnt(struct mount *old, struct dentry *root,
 	struct mount *mnt;
 	int err;
...
+#endif
 ...
 #ifdef CONFIG_FASTUH_KDP
@@ -1224,6 +1406,12 @@ static struct mount *clone_mnt(struct mount *old, struct dentry *root,
 #ifdef CONFIG_FASTUH_KDP
...
@@ -1233,6 +1421,12 @@ static struct mount *clone_mnt(struct mount *old, struct dentry *root,
...
@@ -3691,6 +3885,9 @@ struct mnt_namespace *copy_mnt_ns(unsigned long flags, struct mnt_namespace *ns,
 	namespace_lock();
...
+#endif
 ...
 #ifdef CONFIG_FASTUH_KDP
@@ -4551,6 +4748,36 @@ static struct user_namespace *mntns_owner(struct ns_common *ns)
...
index 7df9ad4d8433..39b7f3c49f55 100644
...
+#endif // #if defined(CONFIG_KSU_SUSFS_SUS_MOUNT) || defined(CONFIG_KSU_SUSFS_SUS_KSTAT)
 {
 	...
 	struct inode *inode;
@@ -84,6 +107,61 @@ static void inotify_fdinfo(struct seq_file *m, struct fsnotify_mark *mark)
...
index 1183fdf77885..648d14bbda29 100644
--- a/fs/proc/base.c
+++ b/fs/proc/base.c
@@ -97,6 +97,9 @@
...
+#endif
 ...
 #include "internal.h"
@@ -1787,6 +1790,10 @@ static const char *proc_pid_get_link(struct dentry *dentry,
 	return ERR_PTR(error);
 }
...
@@ -1796,6 +1803,18 @@ static int do_proc_readlink(struct path *path, char __user *buffer, int buflen)
 	if (!tmp)
 		return -ENOMEM;
...
+
 	...
 	if (IS_ERR(pathname))
@@ -2365,6 +2384,10 @@ proc_map_files_readdir(struct file *file, struct dir_context *ctx)
...
index 81882a13212d..72e0797b47eb 100644
--- a/fs/proc/fd.c
+++ b/fs/proc/fd.c
@@ -12,11 +12,22 @@
 #include <linux/fs.h>
...
@@ -53,9 +64,70 @@ static int seq_show(struct seq_file *m, void *v)
 	if (ret)
 		return ret;
...
index c07365399bf5..1454f845650c 100644
...
@@ -355,6 +358,14 @@ static void show_vma_header_prefix(struct seq_file *m,
 	seq_putc(m, ' ');
 }
...
+
 ...
 {
@@ -367,11 +378,41 @@ show_map_vma(struct seq_file *m, struct vm_area_struct *vma)
 	dev_t dev = 0;
...
+#endif // #ifdef CONFIG_KSU_SUSFS_SUS_KSTAT
 	...
 	start = vma->vm_start;
@@ -382,6 +423,17 @@ show_map_vma(struct seq_file *m, struct vm_area_struct *vma)
...
@@ -930,6 +982,13 @@ static void show_smap_vma(struct seq_file *m, void *v)
...
@@ -997,9 +1056,17 @@ static int show_smaps_rollup(struct seq_file *m, void *v)
...
@@ -1705,6 +1772,9 @@ static ssize_t pagemap_read(struct file *file, char __user *buf,
...
@@ -1714,7 +1784,15 @@ static ssize_t pagemap_read(struct file *file, char __user *buf,
...
index 5b8d065fa83c..bc0637522d2b 100644
...
@@ -239,6 +247,158 @@ static int show_vfsstat(struct seq_file *m, struct vfsmount *mnt)
 	return err;
 }
...
+
 ...
 {
@@ -306,16 +466,34 @@ static int mounts_release(struct inode *inode, struct file *file)
...
index 07a3b5baa404..718a611d7e91 100644
--- a/fs/readdir.c
+++ b/fs/readdir.c
@@ -24,6 +24,11 @@
...
+#endif
 	...
 	if (error >= 0)
 		error = buf.error;
@@ -297,6 +374,10 @@ SYSCALL_DEFINE3(getdents, unsigned int, fd,
 		else
...
@@ -304,6 +385,9 @@ SYSCALL_DEFINE3(getdents, unsigned int, fd,
...
+#endif
 	...
 	buf->error = verify_dirent_name(name, namlen);
 	if (unlikely(buf->error))
@@ -328,6 +415,18 @@ static int filldir64(struct dir_context *ctx, const char *name, int namlen,
 	prev_reclen = buf->prev_reclen;
 	if (prev_reclen && signal_pending(current))
 		return -EINTR;
+#ifdef CONFIG_KSU_SUSFS_SUS_PATH
...
+#endif
 	...
 	prev = (void __user *)dirent - prev_reclen;
 	if (!user_access_begin(prev, reclen + prev_reclen))
@@ -363,6 +462,10 @@ int ksys_getdents64(unsigned int fd, struct linux_dirent64 __user *dirent,
 		.current_dir = dirent
 	};
 	int error;
+#ifdef CONFIG_KSU_SUSFS_SUS_PATH
...
+#endif
 	...
 	if (!access_ok(dirent, count))
 		return -EFAULT;
@@ -371,6 +474,18 @@ int ksys_getdents64(unsigned int fd, struct linux_dirent64 __user *dirent,
 	if (!f.file)
 		return -EBADF;
...
+#endif
 	...
 	if (error >= 0)
 		error = buf.error;
@@ -384,6 +499,10 @@ int ksys_getdents64(unsigned int fd, struct linux_dirent64 __user *dirent,
 		else
 			error = count - buf.count;
 	}
+#ifdef CONFIG_KSU_SUSFS_SUS_PATH
...
@@ -406,6 +525,9 @@ struct compat_old_linux_dirent {
...
@@ -417,6 +539,9 @@ static int compat_fillonedir(struct dir_context *ctx, const char *name,
...
+#endif
 	...
 	if (buf->result)
 		return -EINVAL;
@@ -428,6 +553,18 @@ static int compat_fillonedir(struct dir_context *ctx, const char *name,
 		buf->result = -EOVERFLOW;
 		return -EOVERFLOW;
 	}
+#ifdef CONFIG_KSU_SUSFS_SUS_PATH
...
+#endif
 	...
 	dirent = buf->dirent;
 	if (!access_ok(dirent,
@@ -455,15 +592,35 @@ COMPAT_SYSCALL_DEFINE3(old_readdir, unsigned int, fd,
...
@@ -477,6 +634,9 @@ struct compat_linux_dirent {
...
+#endif
 	...
 	int count;
 	int error;
@@ -491,6 +651,9 @@ static int compat_filldir(struct dir_context *ctx, const char *name, int namlen,
 	compat_ulong_t d_ino;
...
+#endif
 	...
 	if (reclen > buf->count)
@@ -500,6 +663,18 @@ static int compat_filldir(struct dir_context *ctx, const char *name, int namlen,
 		buf->error = -EOVERFLOW;
 		return -EOVERFLOW;
 	}
+#ifdef CONFIG_KSU_SUSFS_SUS_PATH
...
@@ -539,6 +714,10 @@ COMPAT_SYSCALL_DEFINE3(getdents, unsigned int, fd,
 		.count = count
 	};
 	int error;
+#ifdef CONFIG_KSU_SUSFS_SUS_PATH
...
+#endif
 	...
 	if (!access_ok(dirent, count))
 		return -EFAULT;
@@ -547,6 +726,18 @@ COMPAT_SYSCALL_DEFINE3(getdents, unsigned int, fd,
 	if (!f.file)
 		return -EBADF;
...
+#endif
 	...
 	if (error >= 0)
 		error = buf.error;
@@ -558,6 +749,10 @@ COMPAT_SYSCALL_DEFINE3(getdents, unsigned int, fd,
 			error = count - buf.count;
 	}
 	fdput_pos(f);
+#ifdef CONFIG_KSU_SUSFS_SUS_PATH
...
index 298eb77668a7..7edceffce79f 100644
--- a/fs/stat.c
+++ b/fs/stat.c
@@ -17,10 +17,18 @@
...
@@ -77,9 +85,49 @@ int vfs_getattr_nosec(const struct path *path, struct kstat *stat,
...
+#endif // #ifdef CONFIG_KSU_SUSFS_SUS_KSTAT
 	...
 	generic_fillattr(inode, stat);
 	return 0;
@@ -144,6 +192,11 @@ int vfs_statx_fd(unsigned int fd, struct kstat *stat,
 	if (f.file) {
...
+#endif // #ifdef CONFIG_KSU_SUSFS_SUS_KSTAT
 	...
 	return error;
@@ -165,6 +218,7 @@ EXPORT_SYMBOL(vfs_statx_fd);
  *
...
+
 ...
 {
@@ -189,6 +243,11 @@ int vfs_statx(int dfd, const char __user *filename, int flags,
 		goto out;
...
index 4960c69cc47c..09ffa929d2b3 100644
--- a/fs/statfs.c
+++ b/fs/statfs.c
@@ -9,8 +9,20 @@
 #include <linux/security.h>
...
+
 ...
 {
 	int flags = 0;
@@ -67,6 +79,28 @@ static int statfs_by_dentry(struct dentry *dentry, struct kstatfs *buf)
 	return retval;
 }
...
+
 ...
 {
 	struct kstatfs st;
@@ -85,7 +119,40 @@ int vfs_statfs(const struct path *path, struct kstatfs *buf)
 {
 	int error;
...
index 50e2188b12d9..a9373022f916 100644
--- a/fs/super.c
+++ b/fs/super.c
@@ -37,9 +37,17 @@
...
@@ -1093,6 +1101,22 @@ int get_anon_bdev(dev_t *p)
 {
 	int dev;
...
index 000000000000..5397d41baafb
--- /dev/null
+++ b/fs/susfs.c
@@ -0,0 +1,1615 @@
...
index e1a023992673..17d7c829d3bd 100644
...
new file mode 100644
index 000000000000..e54b5a4e2e6f
--- /dev/null
...
new file mode 100644
index 000000000000..4b86f744fc68
--- /dev/null
...
index 14de0c7bf756..5e2fb685eff8 100644
--- a/kernel/kallsyms.c
+++ b/kernel/kallsyms.c
@@ -25,6 +25,9 @@
...
index 9f166d423050..4b98efb740c8 100644
--- a/kernel/sys.c
+++ b/kernel/sys.c
@@ -1254,12 +1254,20 @@ static int override_release(char __user *release, size_t len)
 	return ret;
 }
...
index 1d2ed2df81bc..ceb920fa8e4c 100644
--- a/mm/memory.c
+++ b/mm/memory.c
@@ -73,6 +73,10 @@
 #include <linux/oom.h>
...
+
 ...
 #include <asm/io.h>
@@ -4962,12 +4966,19 @@ int __access_remote_vm(struct task_struct *tsk, struct mm_struct *mm,
